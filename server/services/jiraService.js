...
const searchJira = async (query) => {
  try {
    ...
    if (!JIRA_BASE_URL || !JIRA_USERNAME || !JIRA_API_TOKEN) {
      ...
      return getMockJiraData(query);
    }

    // Search Jira API using JQL
...
const cacheResults = (results) => {
  return new Promise((resolve, reject) => {
    const stmt = db.prepare(
      'INSERT OR REPLACE INTO jira_cache (issue_key, summary, description, status, priority, assignee, reporter, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    );
...
const getMockJiraData = (query) => {
  const mockData = [
    {
      issue_key: 'IT-1234',
      summary: 'Network connectivity issues in Building A',
      description: 'Users in Building A are experiencing intermittent network connectivity issues. Symptoms include slow internet speeds and occasional disconnections.',
      status: 'In Progress',
      priority: 'High',
      assignee: 'John Smith',
      reporter: 'Jane Doe',
    ...
    {
      issue_key: 'IT-1235',
      summary: 'VPN connection timeout errors',
      description: 'Multiple users reporting VPN connection timeout errors when trying to connect from remote locations.',
      status: 'Open',
      priority: 'Medium',
    ...
    {
      issue_key: 'IT-1236',
      summary: 'Email client configuration problems',
      description: 'New employees having trouble configuring their email clients with the correct IMAP/SMTP settings.',
      status: 'Resolved',
      priority: 'Low',
      assignee: 'Lisa Brown',
    ...
    {
      issue_key: 'IT-1237',
      summary: 'Printer driver installation issues',
      description: 'Windows 11 users unable to install printer drivers for HP LaserJet printers.',
      status: 'In Progress',
      priority: 'Medium',
      assignee: 'David Lee',
...
const getIssueDetails = async (issueKey) => {
  try {
    ...
    const cached = await new Promise((resolve, reject) => {
      db.get(
        'SELECT * FROM jira_cache WHERE issue_key = ?',
        [issueKey],
        (err, row) => {
          if (err) reject(err);
          else resolve(row);
    ...
    const response = await jiraApi.get(`/rest/api/3/issue/${issueKey}`, {
      params: {
        fields: 'summary,description,status,priority,assignee,reporter,updated,comment'
      }
    });
